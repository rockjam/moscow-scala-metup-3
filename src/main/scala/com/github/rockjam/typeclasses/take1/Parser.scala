...
package com.github.rockjam.typeclasses.take1

import scala.util.Try

/**
 * Самая базовая реализация typeclass pattern
 * встроенными средствами scala.
 */
...
object Parser {
  ...
  implicit def pairParser[A, B](implicit aParser: Parser[A], bParser: Parser[B]) = new Parser[(A, B)] {
    def parse = { s =>
      val arr = s split ","
      for {
        a <- arr.lift(0)
