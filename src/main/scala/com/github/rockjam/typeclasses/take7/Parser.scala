...
object Parser extends ProductTypeClassCompanion[Parser] {
  ...
  object typeClass extends ProductTypeClass[Parser] {
    override def product[H, T <: HList](ch: Parser[H], ct: Parser[T]): Parser[H :: T] = { from =>
    ...
    override def project[F, G](instance: => Parser[G], to: (F) => G, from: (G) => F): Parser[F] = { s =>
      instance.parse(s).map(from)
    }
  }
